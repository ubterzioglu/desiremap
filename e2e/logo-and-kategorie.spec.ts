...
test.describe('/kategorie/[slug] routes', () => {
  test('bordelle route loads and shows h1', async ({ page }) => {
    const res = await page.goto('http://localhost:3000/kategorie/bordelle')
  ...
  test('fkk-clubs route loads and shows h1', async ({ page }) => {
    const res = await page.goto('http://localhost:3000/kategorie/fkk-clubs')
    expect(res?.status()).toBe(200)
    await expect(page.locator('h1')).toBeVisible()
  ...
  test('kategorie page has search input', async ({ page }) => {
    await page.goto('http://localhost:3000/kategorie/bordelle')
