  ...
  return (
    <section className="relative py-24 bg-gradient-to-b from-[#0f0f14] to-[#0a0a0f] overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-12"><Button onClick={() => setSelectedCategory(null)} size="sm" variant={selectedCategory === null ? 'default' : 'outline'} className={cn('rounded-full px-5', selectedCategory === null ? 'bg-gradient-to-r from-[#8b1a4a] to-[#6b3fa0] text-white border-0' : 'border-white/10 text-gray-300 hover:bg-white/5 hover:text-white')}>Alle</Button>{categories.map((cat) => <Button key={cat.id} onClick={() => setSelectedCategory(cat.id)} size="sm" variant={selectedCategory === cat.id ? 'default' : 'outline'} className={cn('rounded-full px-5', selectedCategory === cat.id ? 'bg-gradient-to-r from-[#8b1a4a] to-[#6b3fa0] text-white border-0' : 'border-white/10 text-gray-300 hover:bg-white/5 hover:text-white')}>{cat.name}</Button>)}</div>
